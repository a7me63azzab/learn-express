...
module.exports = function (app) {
  ...
  app.post('/user/send', (req, res) => {
    ...
    if (!req.body.phoneNum) return res.status(400).send({
      message: 'there is no phone number !'
    })
  ...
  app.post('/user/verify', (req, res) => {
    ...
    if (!req.body.verificationCode) return res.status(400).send({
      message: 'there is no verification Code !'
    });
    User.findOne({
